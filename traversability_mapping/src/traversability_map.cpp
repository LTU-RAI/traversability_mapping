...
private:
    ...
    ros::Publisher pubOccupancyMapGlobal;
    ...
    elevation_msgs::OccupancyElevation occupancyMap2DHeightGlobal; // customized message that includes occupancy map and elevation info
    ...
    PointType globalMapOriginPoint;
    grid_t GlobalMapOriginGrid;
...
public:
    TraversabilityMapping():
        ...
        pubOccupancyMapGlobal = nh.advertise<nav_msgs::OccupancyGrid> ("/occupancy_map_global", 5);
    ...
    void publishGlobalMap(){
        ...
        if (pubOccupancyMapGlobal.getNumSubscribers() == 0){
            return;
        }
        
        // local map origin x and y
        globalMapOriginPoint.x = robotPoint.x - globalMapUpdateLength / 2;
        globalMapOriginPoint.y = robotPoint.y - globalMapUpdateLength / 2;
        globalMapOriginPoint.z = robotPoint.z;
        // local map origin cube id (in global map)
        GlobalMapOriginGrid.cubeX = int((globalMapOriginPoint.x + mapCubeLength/2.0) / mapCubeLength) + rootCubeIndex;
        GlobalMapOriginGrid.cubeY = int((globalMapOriginPoint.y + mapCubeLength/2.0) / mapCubeLength) + rootCubeIndex;
        if (globalMapOriginPoint.x + mapCubeLength/2.0 < 0)  --GlobalMapOriginGrid.cubeX;
        if (globalMapOriginPoint.y + mapCubeLength/2.0 < 0)  --GlobalMapOriginGrid.cubeY;
        // local map origin grid id (in sub-map)
        float originCubeOriginX, originCubeOriginY; // the orign of submap that the local map origin belongs to (note the submap may not be created yet, cannot use originX and originY)
        originCubeOriginX = (GlobalMapOriginGrid.cubeX - rootCubeIndex) * mapCubeLength - mapCubeLength/2.0;
        originCubeOriginY = (GlobalMapOriginGrid.cubeY - rootCubeIndex) * mapCubeLength - mapCubeLength/2.0;
        GlobalMapOriginGrid.gridX = int((globalMapOriginPoint.x - originCubeOriginX) / mapResolution);
        GlobalMapOriginGrid.gridY = int((globalMapOriginPoint.y - originCubeOriginY) / mapResolution);

        int startX=GlobalMapOriginGrid.gridX+GlobalMapOriginGrid.cubeX*mapCubeLength/mapResolution;
        int startY=GlobalMapOriginGrid.gridY+GlobalMapOriginGrid.cubeY*mapCubeLength/mapResolution;
        int targetMaxIndexGMapX=std::min(int(startX+globalMapUpdateLength/mapResolution),globalMapArrayLength); 
        int targetMaxIndexGMapY=std::min(int(startY+globalMapUpdateLength/mapResolution),globalMapArrayLength); 

        // 2 Prepare the occupancyMap2DHeightGlobal messages
        occupancyMap2DHeightGlobal.header.stamp = ros::Time::now();
        occupancyMap2DHeightGlobal.occupancy.header.stamp = occupancyMap2DHeightGlobal.header.stamp;

        // extract all info
        for (int i = startX; i <= targetMaxIndexGMapX; ++i){
            for (int j = startY; j <= targetMaxIndexGMapY; ++j){

                grid_t thisGrid;

                thisGrid.cubeX = i / mapCubeArrayLength;
                thisGrid.cubeY = j / mapCubeArrayLength;

                thisGrid.gridX = i % mapCubeArrayLength;
                thisGrid.gridY = j % mapCubeArrayLength;
                ...
                // skip unknown grid
                if (thisCell->elevation != -FLT_MAX){
                    int index = i + j * globalMapArrayLength; // index of the 1-D array 
                    occupancyMap2DHeightGlobal.occupancy.data[index] = thisCell->occupancy > 80 ? 100 : 0;
                }
            }
        }

        pubOccupancyMapGlobal.publish(occupancyMap2DHeightGlobal.occupancy);
    }
    

    void initializeGlobalOccupancyMap(){
        // initialization of customized map message

        occupancyMap2DHeightGlobal.header.frame_id = "map";
        occupancyMap2DHeightGlobal.occupancy.info.width = globalMapArrayLength;
        occupancyMap2DHeightGlobal.occupancy.info.height = globalMapArrayLength;
        occupancyMap2DHeightGlobal.occupancy.info.resolution = mapResolution;
        
        occupancyMap2DHeightGlobal.occupancy.info.origin.orientation.x = 0.0;
        occupancyMap2DHeightGlobal.occupancy.info.origin.orientation.y = 0.0;
        occupancyMap2DHeightGlobal.occupancy.info.origin.orientation.z = 0.0;
        occupancyMap2DHeightGlobal.occupancy.info.origin.orientation.w = 1.0;

        occupancyMap2DHeightGlobal.occupancy.info.origin.position.x = -(globalMapArrayLength*mapResolution)/2;
        occupancyMap2DHeightGlobal.occupancy.info.origin.position.y = -(globalMapArrayLength*mapResolution)/2;
        occupancyMap2DHeightGlobal.occupancy.info.origin.position.z = 10; // add 10, just for visualization

        occupancyMap2DHeightGlobal.occupancy.data.resize(occupancyMap2DHeightGlobal.occupancy.info.width * occupancyMap2DHeightGlobal.occupancy.info.height);
        occupancyMap2DHeightGlobal.height.resize(occupancyMap2DHeightGlobal.occupancy.info.width * occupancyMap2DHeightGlobal.occupancy.info.height);
        occupancyMap2DHeightGlobal.costMap.resize(occupancyMap2DHeightGlobal.occupancy.info.width * occupancyMap2DHeightGlobal.occupancy.info.height);

        // 1.3 Initialize global occupancy grid map to unknown, height to -FLT_MAX
        std::fill(occupancyMap2DHeightGlobal.occupancy.data.begin(), occupancyMap2DHeightGlobal.occupancy.data.end(), -1);
        std::fill(occupancyMap2DHeightGlobal.height.begin(), occupancyMap2DHeightGlobal.height.end(), -FLT_MAX);
        std::fill(occupancyMap2DHeightGlobal.costMap.begin(), occupancyMap2DHeightGlobal.costMap.end(), 0);
    }
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ////////////////////////////////////////////// Point Cloud /////////////////////////////////////////////////////
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
